...
export async function POST(req: Request) {
  ...
  const prompt = buildPrompt(shot.prompt, shot.cameraPreset as CameraPresetId);
  ...
  // Fire async generation (keeps API responsive; poll via /api/generations/[id])
  void (async () => {
    try {
      const urls: string[] = [];
      ...
      await prisma.generation.update({
        ...
        data: { status: "succeeded", resultUrls: JSON.stringify(urls) },
      });
    } catch (e) {
      ...
      await refundQuota(user!.id, cost);
    }
  })();
